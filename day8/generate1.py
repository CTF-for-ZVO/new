...
c2_session = {
    ...
    "cat /root/flag.txt": "CTF{D0uble_Tunnel_F0und_By_BLUETEAM}"
}
...
def generate_packet(src_ip, dst_ip, src_port, dst_port, payload, timestamp):
    ...
    global DNS_ID
    ether_layer = Ether(src="00:11:22:33:44:55", dst="AA:BB:CC:DD:EE:FF")
...
# Завантаження / Генерація фонового шуму (зверніть увагу: всі пакети повинні мати Ether()!)
try:
    noise_packets = rdpcap("background_noise.pcap") 
except FileNotFoundError:
    print("\nПопередження: файл background_noise.pcap не знайдено. Генеруємо фіктивний шум.")
    noise_packets = []
    legit_domains = ["google.com.", "microsoft.com.", "update.local.net."]
    for _ in range(30):
        domain = random.choice(legit_domains)
        ip_layer = IP(src=VICTIM_IP, dst=DNS_SERVER_IP)
        udp_layer = UDP(sport=random.randint(40000, 60000), dport=53)
        dns_query = DNS(qd=DNSQR(qname=domain, qtype="A"))
    
        noise_pkt = Ether() / ip_layer / udp_layer / dns_query
        noise_pkt.time = timestamp + random.uniform(0.1, 40.0)
        noise_packets.append(noise_pkt)
